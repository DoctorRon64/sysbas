...
class Client {
  ...
  startClient(address,port) {
    if (connected) {
      const sendData = {"ip":address,"port":port,"id": socket.id};
      this.port = port;
  ...
  sendMessage(...data) {
    ...
    if (this.clientActive) {
      if (message === undefined || message.length === 0) {
        ...
        const sendData = {"address":address,"message":message,"id": socket.id,"port":this.port};
...
class Server {
  ...
  startServer(port) {
    if (connected) {
      const sendData = {"port":port,"id": socket.id};
...
function makeNote(note=60,velo=0.5,dur=100.0) {
  ...
  if (getAudioContext().state != 'running') {
    ...
    polySynth.play(midiToFreq(note), velo, 0.00075, dur / 1000.);
  }
}
